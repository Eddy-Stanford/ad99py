...
def save_loon_basins():
    ...
    (
        ...
        trop_pac_flights,
        SO_flights,
    ) = process_flights(loon_data, masks)
    ...
    return (
        trop_atl_flights_path,
        extra_atl_flights_path,
        extra_pac_flights_path,
        indian_flights_path,
        trop_pac_flights_path,
        SO_flights_path,
    )
...
def get_loon_basin_data(basin: str):
    ...
    if not path_name.exists():
        print(
            f"[INFO] Basin data for '{basin}' not found in cache. Generating basin data..."
        )
        save_loon_basins()
    return path_name
