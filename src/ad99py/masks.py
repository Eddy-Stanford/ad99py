...
def load_mask(
    path: Optional[str] = None, recentering: bool = True, **kwargs
) -> xr.Dataset:
    if path is None:
...
def mask_dataset(
    ds: xr.Dataset, basins=None, mask: Optional[xr.Dataset] = None, **kwargs
):
    if mask is None:
        mask = load_mask(**kwargs)
    if "latitude" in ds.variables and "longitude" in ds.variables:
        ...
        if basins is None:
            ...
            try:
                ...
                raise KeyError(
                    f"No basin named {e.args[0]}. Allowed basins are {set(interp_mask.data_vars.keys())}"
                ) from e
        ...
        raise KeyError(
            "Dataset must have 'latitude'/'longitude' or 'lat'/'lon' coordinates."
        )
