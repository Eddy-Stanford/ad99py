...
def flights_to_numpy_array(flight):
    return np.stack([flight.flux_west.values, flight.flux_east.values,flight.flux_south.values,flight.flux_north.values])*1000 
...
def process_flights(flight_path,masks):
    print("[INFO] Processing Loon flight data and applying masks for all basins. This may take some time...")
    flights = pd.read_csv(flight_path, parse_dates=['time'], infer_datetime_format=True,dtype=LOON_DTYPES, usecols=LOON_DTYPES.keys())
    ...
    return (flights_to_numpy_array(trop_atl_flights),
